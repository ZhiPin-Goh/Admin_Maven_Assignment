        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden bg-[#f6f8f6]">
          <header class="px-8 py-6 shrink-0 bg-white border-b border-gray-200 shadow-sm">
            <div class="max-w-[1400px] mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-6">
              <div>
                <h2 class="text-3xl font-black text-[#0b3e2e] tracking-tight">Customer List</h2>
              ...
              <div class="flex gap-4">
                ...
                <button class="flex items-center gap-2 bg-[#f6f8f6] border border-[#cfe7d7] text-[#0b3e2e] px-4 py-2.5 rounded-lg hover:bg-green-100 transition-colors shadow-sm text-sm font-medium">
                          ...
                          <tr class="group hover:bg-green-50/50 transition-colors">
                            <td class="px-6 py-4">
                              <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-[#2ee59d]/20 text-green-800 flex items-center justify-center text-sm font-bold uppercase">
                                   ${user.userName.substring(0, 2)}
                                </div>
                                <span class="text-sm font-bold text-[#0b3e2e]">${user.userName}</span>
                              </div>
                            </td>
                                ...
                                <c:choose>
                                    <c:when test="${user.status == 'Active'}">
                                        <form action="inactiveUser" method="post" class="m-0">
                                            ...
                                            <button type="submit" class="w-11 h-6 bg-[#2ee59d] rounded-full relative cursor-pointer border-none p-0 outline-none focus:ring-2 focus:ring-[#2ee59d] focus:ring-offset-1 transition-all" aria-label="Set user inactive">
